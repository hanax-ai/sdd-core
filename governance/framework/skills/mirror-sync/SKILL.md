...
- **Global:** `reference/repos/` (workspace root) — framework source/doc mirrors shared by
  all projects.
- **Project:** `governance/framework/reference/` — project-specific mirrors (e.g.
...
1. **Project overrides first:** the Markdown registry table in
   `governance/framework/knowledge/instructions.md` §4 (Local Mirror Overrides) — within
   this internal domain, its rows override same-named global rows.
