...
- [x] T004 Contract test: `get_message` deterministic per `(month, day)` (FR-001) — `tests/unit/test_api.py` *(written; failed as required before T008)*
- [x] T005 Contract test: invalid month/day pair raises descriptive error (FR-003) — `tests/unit/test_api.py::test_invalid` *(failed before implementation; same file as T004: sequential)*
...
- [x] T009 `get_message(month, day)` with validation — `src/motd/api.py`
