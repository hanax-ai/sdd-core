...
# 1. Deterministic message for an explicit date (FR-001)
assert get_message(date(2026, 1, 1)) == get_message(date(2026, 1, 1))
...
assert get_message_today(FixedClock()) == get_message(date(2026, 3, 1))
```

Invalid dates raise a descriptive `ValueError` (FR-003). That is the entire public surface.
