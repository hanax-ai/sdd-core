...
- **Validation rules**: every calendar day present exactly once (366 keys incl. `(2, 29)`); every value non-empty, no user data, no external content (FR-004); a given `(month, day)` returns the same message in every year (FR-001, R-002).
