...
**Feature Directory**: `[adopter-root]/docs/specs/[###-feature-name]/` | **Date**: [DATE] | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `[adopter-root]/docs/specs/[###-feature-name]/spec.md`
...
3. Read BOTH constitutions:
   ...
   → Project: [adopter-root]/.specify/memory/constitution.md
...
**Storage**: [per the governing plan's reviewed storage decision (root Article II), e.g., Git-managed files, project datastore, or N/A — datastores appear here only as this plan's reviewed storage decision, never as a fixed stack]

**Validation Mode**: `test-runtime` <!-- pinned for the software/product template: classic TDD — tests are written and fail before the implementation they cover -->
...
### Project Constitution Gate — `[adopter-root]/.specify/memory/constitution.md`
...
```text
[adopter-root]/docs/specs/[###-feature-name]/
...
2. **Resolve each unknown by reading workspace knowledge first**:
   ...
   - Project guidance: `[adopter-root]/knowledge/instructions.md`, `[adopter-root]/reference/`
...
5. **Record new agent context in project knowledge**: if design introduces technologies or conventions future agents must know, append them to `[adopter-root]/knowledge/instructions.md` directly — keep additions incremental and preserve existing content
...
- Use `governance/framework/docs/specs/template-software/tasks.md` as the structural base
...
*Gated by `.specify/memory/constitution.md` (workspace root) and `[adopter-root]/.specify/memory/constitution.md` — see the Constitution Check above.*
