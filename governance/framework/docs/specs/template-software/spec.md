...
1. Start a new SOFTWARE/PRODUCT feature by COPYING this template folder:
   governance/framework/docs/specs/template-software/  ->  <adopter-root>/docs/specs/[###-feature-name]/
   (This is the workspace's single canonical software/product template home,
   for future product projects. Use the next available zero-padded number.)
2. Fill in the metadata block above (Feature Folder, Created, Input).
3. Read the governing context BEFORE writing any content:
   -> Adopter constitution: [adopter-root]/.specify/memory/constitution.md
...
- [ ] Spec conforms to the adopter constitution (`[adopter-root]/.specify/memory/constitution.md`)
