...
UNINITIALIZED
  -> INSPECTING
  -> BLOCKED | READY_READ_ONLY | DEGRADED
READY_READ_ONLY
...
| Workflow is unavailable without both an adopted degraded policy and a valid pre-issued mission envelope | `BLOCKED` | Both degradation prerequisites are mandatory |
| Workflow is unavailable with both an adopted degraded policy and a valid pre-issued mission envelope | `DEGRADED` | Only the pre-issued mission's already-authorized scope may proceed |
