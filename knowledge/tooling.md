...
| wip policy | workspace-native skills | required | global (workspace) | `wip/README.md` (+ TEMPLATE.md) | — (authored in-workspace) | Agent Zero directive 2026-07-19 (post-plan) | NON-AUTHORITATIVE brainstorming space: exploratory only, zero implementation authority; two-gate approval (promotion approved-for-specification, then implementation approval on the resulting spec/plan), item-and-action-specific. Item content + _index.md git-ignored; README/TEMPLATE committed. Article III: root-scope writes only. |
