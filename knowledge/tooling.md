...
| wip policy | workspace governance | required | global (workspace) | `wip/README.md` (+ TEMPLATE.md) | — (authored in-workspace) | Agent Zero directive 2026-07-19 (post-plan) | NON-AUTHORITATIVE brainstorming space: exploratory only, zero implementation authority; two-gate approval (promotion approved-for-promotion, then implementation approval on the resulting spec/plan), item-and-action-specific. Item content + _index.md git-ignored; README/TEMPLATE committed. Article III: root-scope writes only. Category `workspace governance` introduced post-constitution-v1.1.0 (policy artifact, not a skill); constitutional category-list refresh queued for the next amendment. |
