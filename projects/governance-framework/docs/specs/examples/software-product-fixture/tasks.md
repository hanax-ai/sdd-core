...
- [x] T003 [P] Add lint configuration — `pyproject.toml`

## Phase 3.2: Tests First (TDD) ⚠️ MUST COMPLETE BEFORE 3.3

**CRITICAL: These tests MUST be written and MUST FAIL before ANY implementation**

- [x] T004 [P] Contract test: `get_message` deterministic per date (FR-001) — `tests/unit/test_api.py` *(written; failed as required before T008)*
- [x] T005 [P] Contract test: invalid date raises descriptive error (FR-003) — `tests/unit/test_api.py::test_invalid` *(failed before implementation)*
- [x] T006 [P] Contract test: injected-clock current-day path (FR-002) — `tests/unit/test_api.py::test_today` *(failed before implementation)*
...
- [x] T008 [P] MessageCatalog with 366 entries incl. leap day — `src/motd/catalog.py`
...
- [x] T012 [P] Edge-case unit tests (leap day, year boundaries); full suite green — `tests/unit/test_catalog.py`
...
- Tests (T004–T007) before implementation (T008–T010); each was observed FAILING first
