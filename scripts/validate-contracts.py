...
def load_validation_dependencies() -> tuple[Any, Any, Any, Any]:
    try:
        import yaml as yaml_module
        from defusedxml import ElementTree as element_tree_module
        from defusedxml.common import DefusedXmlException as xml_exception_class
        ...
        with tempfile.TemporaryDirectory(prefix="sdd-core-validation-") as temp_root:
            environment = Path(temp_root) / "venv"
            python = environment / (
            ...
            completed = subprocess.run(
                [str(python), str(Path(__file__)), *sys.argv[1:]]
            )
            raise SystemExit(completed.returncode)


yaml, Draft202012Validator, ElementTree, DefusedXmlException = (
    load_validation_dependencies()
)
SCHEMAS = {
...
def adoption_semantic_errors(instance: dict[str, Any]) -> list[str]:
    tailored_ids = [item["id"] for item in instance["provisions"]["tailored"]]
    errors: list[str] = []
    if len(tailored_ids) != len(set(tailored_ids)):
        errors.append("tailored provision IDs must be unique")
    overlap = set(tailored_ids) & set(instance["provisions"]["excluded"])
    if overlap:
        errors.append(
            "provisions cannot be both tailored and excluded: "
            + ", ".join(sorted(overlap))
        )
    return errors
...
def validate_contract_slice(name: str) -> list[str]:
    ...
    for path in sorted((fixture_root / "valid").glob("*.json")):
        ...
        semantic = (
            ...
            if name == "evidence"
            else adoption_semantic_errors(instance)
            if name == "adoption"
            else []
        )
    ...
    for path in sorted((fixture_root / "invalid").glob("*.json")):
        ...
        elif not errors and name == "adoption":
            semantic = adoption_semantic_errors(instance)
...
def validate_template() -> list[str]:
    ...
    instance = load_yaml(path)
    errors = list(validator.iter_errors(instance))
    semantic = adoption_semantic_errors(instance) if not errors else []
    if errors or semantic:
        return [
            f"{path.relative_to(ROOT)} expected PASS: {errors!s} {semantic!s}"
        ]
...
def repository_markdown_files() -> list[Path]:
    ...
    for path in ROOT.rglob("*.md"):
        ...
        if relative.parts[:2] == ("reference", "repos"):
            continue
        if relative.parts[:1] == (".superpowers",):
            continue
...
def validate_markdown_links() -> list[str]:
    failures: list[str] = []
    copied_software_template = (
        ROOT / "governance/framework/docs/specs/template-software"
    )
    copied_feature_parent = ROOT / "docs/specs/example-feature"
    for path in repository_markdown_files():
        ...
        for raw_target in MARKDOWN_LINK.findall(text):
            ...
            if not target or any(marker in target for marker in ("{", "}", "*")):
                continue
            if path.parent == copied_software_template:
                copied_resolved = (
                    copied_feature_parent / unquote(target)
                ).resolve()
                try:
                    copied_relative = copied_resolved.relative_to(
                        copied_feature_parent
                    )
                except ValueError:
                    resolved = copied_resolved
                else:
                    resolved = copied_software_template / copied_relative
            else:
...
def validate_structured_files() -> list[str]:
    ...
    unsafe_svg_fixture = ROOT / "scripts/fixtures/invalid/svg-dtd-entity.svg"
    for path in ROOT.rglob("*"):
        if not path.is_file():
            continue
        if path.relative_to(ROOT).parts[:1] == (".superpowers",):
            continue
        if path == unsafe_svg_fixture:
            continue
        try:
...
def validate_svg_security() -> list[str]:
    fixture = ROOT / "scripts/fixtures/invalid/svg-dtd-entity.svg"
    try:
        ElementTree.parse(fixture)
    except DefusedXmlException:
        return []
    except Exception as exc:
        return [
            f"{fixture.relative_to(ROOT)} rejected by an unexpected parser error: {exc}"
        ]
    return [
        f"{fixture.relative_to(ROOT)} expected DTD/entity rejection but parsed"
    ]
